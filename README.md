...
2. **resolve-release-tag** — writes `.release-tag` by reading the semver **git tag already on this commit** (created by the push pipeline's `tag-release-auto-dev` after its build + e2e + dev deploy). Deployments **never allocate** a version — that is `compute-version`'s job on the push pipeline only. Fails closed if the commit has no release tag (never built/dev-deployed on `main`).
